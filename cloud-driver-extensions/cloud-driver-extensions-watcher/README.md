...
- `watch()` hardcodes the exact Postgres schema `database-driver-plugin 1.3.13`'s `SQLDatabaseSection` happens to create (`id TEXT, data BYTEA`) - this is an implementation detail of that upstream artifact, not a published cross-version contract, and needs re-verifying whenever its pinned version changes.
...
- **`PostgresDatabaseNotification`/`DatabaseNotification`** (upstream, `database-driver-plugin`/`database-driver-api`, pinned `1.3.13`) - the actual `LISTEN`/`NOTIFY` mechanics, not part of this repo at all. This module only wires that vendor class up as an `Extension` and supplies the callback.
