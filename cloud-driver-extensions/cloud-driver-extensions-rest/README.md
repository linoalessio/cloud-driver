...
3. Builds `Argon2idPasswordHasher` + `JjwtSigner` + `AuthService` + `CloudUserService`, then constructs `new DefaultRestFactory(dataFactory, authService, cloudUserService)`.
