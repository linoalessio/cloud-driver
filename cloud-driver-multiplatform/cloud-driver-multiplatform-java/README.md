...
**Moved, then renamed (2026-09-07)**: this module used to be `cloud-driver-platforms-rest`, a child of the `cloud-driver-platforms` aggregator. It first moved to `cloud-driver-multiplatform/cloud-driver-maven`, then - the same day - was renamed again to its current name/path, `cloud-driver-multiplatform/cloud-driver-multiplatform-java` — `cloud-driver-multiplatform`'s Java/Maven sibling of `cloud-driver-multiplatform-python` (Python) and `cloud-driver-multiplatform-swift` (Swift), all three being client SDKs for the same REST/WebSocket API, one per ecosystem. Only the Maven module name/coordinates/parent changed across both moves; the Java package root (`de.lino.cloud.platform.rest`) is unchanged on purpose (see below) and `cloud-driver-platforms-desktop` still depends on this module exactly as before, just under its new Maven coordinates.

## Project structure

Maven `packaging=jar`, `groupId=de.lino.cloud.multiplatform.java`, `artifactId=cloud-driver-multiplatform-java`, module/package root `de.lino.cloud.platform.rest` (a deliberate holdover from this module's pre-move name/history — renaming the Maven module didn't rename the Java package, since that would touch every import across `cloud-driver-platforms-desktop` and this module's own source for no functional benefit; see `MacKeychainTokenStore`'s own even older holdover, `"de.lino.cloud.platform.app"`, for the same reasoning applied a second time). Its Maven `<parent>` is the `cloud-driver-multiplatform` aggregator (`../pom.xml`), which is itself a plain child `<module>` of the repo root `pom.xml`.
