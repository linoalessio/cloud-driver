...
`cloud-driver-bootstrap-1.0.2.jar` (shaded, runnable) is built via:

```
mvn -pl cloud-driver-bootstrap -am package
java -jar cloud-driver-bootstrap-1.0.2.jar
```
...
self-contained, runnable `cloud-driver-bootstrap-1.0.2.jar` with every dependency shaded in and a
...
The plain `maven-jar-plugin` output (`original-cloud-driver-bootstrap-1.0.2.jar`) is kept
