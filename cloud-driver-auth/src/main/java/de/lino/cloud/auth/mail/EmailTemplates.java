...
/**
 * Builds the HTML/plain-text pair for every e-mail {@code AuthService} sends - the verification
 * mails (registration, password reset, e-mail change) and the notifications that tell an address
 * something has already happened. One shared shape (logo header, headline, greeting, intro
 * paragraph, a highlighted verification-code callout on the verification pair only, a notice
 * paragraph, footer) parameterized per call site, rather than a near-duplicate template each. Styled after
...
public final class EmailTemplates {
    ...
    /**
     * Builds the HTML body for a notification e-mail - the same card shape {@link
     * #buildVerificationHtml} renders, without the verification-code callout. For a message that
     * tells an address something has happened, rather than asking it to prove it controls
     * anything: there is no code to show, and rendering an empty callout box would read as a
     * broken mail.
     *
     * @param headline the large heading at the top of the card, e.g. {@code "Your password was changed."}
     * @param greetingAddress the e-mail address the "Hello ..." greeting line addresses
     ...
     * @return a complete, self-contained HTML document
     */
    public static String buildNoticeHtml(final String headline, final String greetingAddress, final String introText,
    ...
    /**
     * Builds the plain-text fallback body for the same e-mail {@link #buildNoticeHtml} renders -
     * a notification with no verification code, so no code line is emitted at all.
     *
     * @param greetingAddress the e-mail address the "Hello ..." greeting line addresses
     * @param introText the paragraph explaining what happened
     * @param noticeText the closing paragraph (what to do if this was not the recipient)
     * @return the plain-text body
     */
    public static String buildNoticePlainText(final String greetingAddress, final String introText,
                                              final String noticeText) {
        return String.join("\n",
                "Hello " + greetingAddress + ",",
                "",
     ...
     * escaping library here), since the only dynamic values ever passed through are
     * already-validated e-mail addresses, a 6-digit numeric code, and the fixed prose each call
     * site supplies.
     *
