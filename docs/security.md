...
| Row identity (the `id` column) | Envelope encryption covers a row's payload, never its `id`, which the persistence layer writes as plain text — and the primary key's copy inside the authenticated data is stored alongside the ciphertext, so it appears in the row twice. No primary key may therefore *be* a credential or personal data. Refresh tokens and the two address-keyed pending rows (a pending registration, a pending password reset) are stored under a lowercase-hex SHA-256 of the presented value; the raw value lives only inside the encrypted payload. The lookup stays a single point read, because the presenter supplies the value and the server hashes it before looking it up. Unsalted deliberately: the lookup has to be deterministic, and the inputs either carry their own entropy (48 bytes of `SecureRandom`) or are supplied in full by the caller. A pending e-mail change keys on the account's own random id, which is neither a credential nor personal data, and needs no digest |
...
flowchart LR
    ...
    CHANGE["Password reset /<br/>e-mail change /<br/>suspension /<br/>forced sign-out"] -->|"all sessions ended"| OUT["Every refresh token revoked<br/>+ every access token refused"]
```
...
- A refresh token is never recoverable from the database or from a backup archive: a dump yields
  digests only, and a digest cannot be presented as a token. A password reset, an e-mail change and
  a suspension each revoke every outstanding refresh token for the account, so a stolen session
  cannot be renewed past the change. Archives taken before refresh tokens were stored this way
  still contain the raw values in their `id` columns — treat those archives as credential-bearing,
  and either purge them or let them age out under the normal retention.
...
- Every access token carries the issuing account's session generation as a claim **in the signed
  payload, never a header**, so it cannot be stripped or rewritten without breaking the signature.
  A password reset, an e-mail change, a suspension and an operator's forced sign-out all advance
  that generation, so every access token issued before the change is refused on its very next
  request instead of living out its 12 hours. The outstanding refresh tokens are revoked in the
  same step and the account's live-update sockets are closed, so the account must sign in again —
  with the new credential. The check lives in token validation itself, so the WebSocket handshake
  inherits it and a closed socket cannot simply be re-opened with the same token.
- Changing an account's e-mail address re-verifies the current password on top of the bearer token
  — an access token on its own can never move an account to an attacker-controlled address, which
  would otherwise turn a temporary credential into permanent ownership through a reset on the new
  address. Both the request and the completed change are announced to the outgoing address, so the
  real owner is told while they can still act. Completing the change revokes every refresh token,
...
| Oversized uploads | A hard request-size ceiling (256 MiB) enforced *mid-stream*: a server-mediated upload is written to a scratch file in 8 KiB chunks and rejected the moment the running total exceeds the ceiling, before the rest of the body is read and without ever buffering it in heap. The per-account upload quota is enforced on top of that — up front, before a single byte moves, on the presigned and resumable paths (which also cap how many sessions one account may hold open at once), and on the received file for a server-mediated upload. An open session's or ticket's declared size is **reserved** against the account's quota for as long as it is open and counts alongside every other open pending upload, so concurrent uploads cannot each pass a check the others invalidate; the reservation is released by completion, by an abort, or by the retention sweep. Part URLs can only be issued for part numbers the session's own geometry contains, so no presigned part can accept bytes the account was never charged for |
...
- In the reference deployment (see `shell/provision-root-server.sh`), **Caddy terminates TLS**
  ...
  process or its configuration. That block also sets `header_up X-Forwarded-For {remote_host}`, so
  the value the backend reads is the address Caddy itself observed rather than anything the client
  sent.
