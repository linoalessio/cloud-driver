...
| ├─ `cloud-driver-extensions-rest` | Stands up the JWT-authenticated `RestFactory` (login, e-mail-verified self-registration, refresh tokens, per-user file/folder CRUD + trash + sharing, admin routes, live push) over Javalin. | [README](cloud-driver-extensions/cloud-driver-extensions-rest/README.md) |
...
| └─ `cloud-driver-platforms-desktop` | **Not a Maven module** (Gradle/Kotlin Multiplatform/Compose Desktop, own `gradlew`) - the real end-user desktop app built on `cloud-driver-platforms-rest`: register/login, browse/upload/download/share files+folders, trash, admin panel. | [README](cloud-driver-platforms/cloud-driver-platforms-desktop/README.md) |
...
- **Refresh tokens and an admin flag both exist.** Each access JWT is
  paired with a longer-lived (30-day), opaque, single-use `RefreshToken` (rotated on every use) so a
  long-running client stays signed in without re-prompting for a password every 12 hours.
  `AuthUser#isAdmin` is a single boolean, written only from a terminal
