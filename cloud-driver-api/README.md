...
├── file/                         file upload/download domain model
│   ├── exception/                FileIntegrityException
│   ├── meta/                     FileChecksum, FileMetadata
│   └── pending/                  PendingUploadCache (offline-safe upload queue contract)
├── jwt/                          end-user (JWT) authentication contracts
│   ├── auth/                     IAuthService
...
└── utility/                      Asserts, Constraints, and (task/) MultiTaskingFactory
```

Every package above follows the same rule: **this module defines the contract, `cloud-driver-plugin`/`cloud-driver-auth` supply the implementation.** The only exceptions - concrete classes that live here because they have nothing but core-Java/Lombok/Gson dependencies - are `StoredFile`, `FileChecksum`, `FileMetadata`, `ApiKey`, `AuthUser`, `EncryptedPayload`, `EnvelopeEncryptedPayload`, `EncryptedEntityRecord`, `WrappedKey`, `DataEncryptionKey`, `ExtensionProperties`/`ExtensionPropertiesLoader`, `AnsiColors`, `Terminal` and its supporting terminal classes, and the small static utility classes.
...
- **`IServiceContainer`** (`de.lino.cloud.api.factory.service`) - a second, smaller bundle for higher-level, cross-cutting services built *on top of* the raw facets above, rather than being one themselves. Holds two accessors, `getCloudUserService()` (returning `de.lino.cloud.api.user.ICloudUserService` - see [The `user` package](#the-user-package-end-user--owned-file-bookkeeping)) and `getAuthService()` (returning `de.lino.cloud.api.jwt.auth.IAuthService`), both of which `RestFactory` itself also exposes directly (`RestFactory#getCloudUserService()`/`RestFactory#getAuthService()`) so its own route handlers and any external caller agree on the same instances.
...
- **`AuthUser`** (`de.lino.cloud.api.jwt.user`) - the persisted account entity: `id` (primary key), `emailAddress`, `passwordHash` (a PHC-style Argon2id string; the raw password is never retained anywhere on this class). Named `AuthUser` rather than plain `User` deliberately, since `USER` is a reserved SQL keyword and an entity literally named `User` produces an unquoted `CREATE TABLE User (...)` Postgres rejects with a syntax error.
- **`IAuthService`** (`de.lino.cloud.api.jwt.auth`) - `register(emailAddress, rawPassword)` (starts registration: validates the address, then e-mails a verification code rather than creating the account), `confirmRegistration(emailAddress, code)` (verifies that code and only then creates the account, returning a signed JWT), `login(emailAddress, rawPassword)` (returns a signed JWT), `validate(jwt)` (returns the embedded user id). Framework-agnostic: `login`/`validate`/`confirmRegistration` throw plain `InvalidCredentialsException`/`InvalidJwtException`/`InvalidVerificationCodeException` (`de.lino.cloud.api.jwt`) rather than an HTTP-specific type, leaving translation into an actual HTTP response to whatever wires this into Javalin. Both `register` and `confirmRegistration` are exposed as `POST /auth/register`/`POST /auth/register/confirm` by `cloud-driver-plugin`'s `DefaultRestFactory` whenever it's constructed with an `AuthService` - this deployment's open, e-mail-verified self-registration flow; there is no separate operator-run account-creation tool.
...
- **`EmailSender`/`EmailDeliveryException`** (`de.lino.cloud.api.mail`) - `send(toAddress, subject, body)`, the contract `AuthService#register` delivers its verification code through. `cloud-driver-auth` supplies the concrete implementations: `SmtpEmailSender` (SMTP+STARTTLS via Jakarta Mail/Angus Mail, the only one meant for production) and `LoggingEmailSender` (logs instead of sending, used as a fallback when no SMTP server is configured).
