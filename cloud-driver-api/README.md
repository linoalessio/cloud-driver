...
- **`IServiceContainer`** (`de.lino.cloud.api.factory.service`) - a second, smaller bundle for higher-level, cross-cutting services built *on top of* the raw facets above, rather than being one themselves. Today it holds exactly one accessor, `getCloudUserService()` (returning `de.lino.cloud.api.user.ICloudUserService` - see [The `user` package](#the-user-package-end-user--owned-file-bookkeeping)), which `RestFactory` itself also exposes directly (`RestFactory#getCloudUserService()`) so its own `/files` route handlers and any external caller agree on the same instance.
...
- **`IAuthService`** (`de.lino.cloud.api.jwt.auth`) - `register(username, rawPassword)`, `login(username, rawPassword)` (returns a signed JWT), `validate(jwt)` (returns the embedded user id). Framework-agnostic: `login`/`validate` throw plain `InvalidCredentialsException`/`InvalidJwtException` (`de.lino.cloud.api.jwt`) rather than an HTTP-specific type, leaving translation into an actual HTTP response to whatever wires this into Javalin. `register` is deliberately not wired to any public HTTP route by this contract - see `CreateUserCli` in `cloud-driver-bootstrap` for how new accounts are actually created.
...
- **`ICloudUser`** (`de.lino.cloud.api.user`) - the behavioral contract a persisted per-user record (`CloudUser` in `cloud-driver-auth`) implements: one method, `getAuthUserId()`, tying the record back to its owning `AuthUser`. File ownership is deliberately *not* part of this contract - it is tracked through dedicated per-(user, file) records instead (`StoredFileOwnership`, in `cloud-driver-auth`), so adding/removing one owned file never requires decrypting and re-encrypting an entire user record.
