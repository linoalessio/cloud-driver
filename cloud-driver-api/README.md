...
│   ├── auth/                     IAuthService, AuthTokens, LoginResult, TwoFactorSetupStart
...
- **`AuthUser`** (`de.lino.cloud.api.jwt.user`) - the persisted account entity: `id` (primary key), `emailAddress`, `passwordHash` (a PHC-style Argon2id string; the raw password is never retained anywhere on this class), `isAdmin` (a single boolean admin flag - deliberately not a roles/permissions system; its only writer anywhere in the codebase is `IAuthService#setAdmin`, never reachable via REST, only from a terminal operator command, to avoid a privilege-escalation hole), and `totpSecretBase32` (nullable - two-factor authentication's RFC 6238 shared secret, `null` meaning 2FA is disabled). Named `AuthUser` rather than plain `User` deliberately, since `USER` is a reserved SQL keyword and an entity literally named `User` produces an unquoted `CREATE TABLE User (...)` Postgres rejects with a syntax error.
- **`IAuthService`** (`de.lino.cloud.api.jwt.auth`) - the full account lifecycle, grouped by concern:
  ...
  - **Login:** `login(emailAddress, rawPassword)` returns a `LoginResult` - either a real `AuthTokens` pair, or (if the account has 2FA enabled) a pending token that must be exchanged, together with a TOTP code, via `completeTwoFactorLogin`. `validate(jwt)` returns the embedded user id.
  - **Two-factor authentication (TOTP):** `beginTwoFactorSetup(authUserId)` (generates a not-yet-live secret + `otpauth://` URI), `confirmTwoFactorSetup(authUserId, code)` (promotes it to live), `disableTwoFactor(authUserId, password)` (re-verifies the password first), `completeTwoFactorLogin(pendingToken, code)`.
