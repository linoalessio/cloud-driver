...
public final class AuthUser extends Serialized {
     ...
     * at {@code false} - every account created through registration starts this way; use
     * {@link #withAdmin(boolean)} to change it afterward.
     *
    ...
    /**
     * Full constructor, used by {@link #withAdmin(boolean)} and by any caller reconstructing an
     * existing account (e.g. after a password/email change) that must carry the admin flag
     * forward rather than silently resetting it.
     *
     * @param id this account's unique id, its {@link #primaryKey()}
     * @param emailAddress this account's identifying email address
     * @param passwordHash a PHC-style Argon2id string produced by {@code PasswordHasher#hash} - never the raw password
